...
import AlertLengkapi from '@/components/angkut/AlertLengkapi.vue'
 ...
 *
 * Dipakai pop-up "Lengkapi dulu" untuk menyebut persis apa yang kurang —
 * pengguna yang hanya lupa mengisi berat tidak perlu membaca daftar semuanya.
 */
const kurang = computed(() => {
  const daftar: string[] = []
  // Nama sengaja ringkas: keempatnya harus muat dalam dua baris kartu alert.
  if (!catatan.value.trim()) daftar.push('detail barang')
  if (!beratTotal.value || beratTotal.value <= 0) daftar.push('berat')
  if (!tanggal.value) daftar.push('tanggal')
  if (!waktu.value) daftar.push('waktu pengambilan')

  return daftar
})

const alertTampil = ref(false)

/**
 * Menyala setelah "Lanjut" ditekan tapi validasinya gagal.
 *
 * Sebelum itu tidak ada satu pun garis merah: halaman yang baru dibuka belum
 * berisi kesalahan apa pun, dan menandainya merah sejak awal membuat pengguna
 * merasa sudah berbuat salah padahal belum menyentuh apa-apa. Bintang merah di
 * labelnya yang menandai isian wajib.
 *
 * Sekali menyala, penanda ini tidak dimatikan lagi — tapi merahnya hilang
 * sendiri per isian begitu isian itu terisi.
 */
...
function lanjut() {
  if (kurang.value.length) {
    ditandai.value = true
    // Ditutup dulu lalu dibuka lagi supaya animasinya terulang kalau tombol
    // ditekan berkali-kali; tanpa ini ketukan kedua tidak terasa direspons.
    alertTampil.value = false
    requestAnimationFrame(() => (alertTampil.value = true))
    return
  }
  ...
  <template v-else>
    <div class="min-h-dvh w-full bg-(--color-surface) text-(--color-on-surface) pb-28">
      ...
      <div class="px-5 py-6 flex flex-col gap-7">
        ...
        <!-- Load details -->
        <section class="flex flex-col gap-3.5">
          <h2 class="text-sm font-bold text-(--color-on-surface)">
            Detail Barang Bawaan <TandaWajib />
          </h2>
          <textarea
            v-model="catatan"
            placeholder="Contoh: 2 kardus besar, 1 meja kecil, dan 1 koper."
            aria-required="true"
            class="w-full rounded-(--radius-input) bg-(--color-surface-container) px-3.5 py-3 text-sm outline-none resize-none min-h-28 border-2 placeholder:text-(--color-on-surface-variant)"
          ...
          <div
            class="flex items-center gap-3 rounded-(--radius-input) bg-white border-2 shadow-xs px-3.5 py-3"
            :class="
              ditandai && !(beratTotal && beratTotal > 0)
                ? 'border-(--color-error)'
                : 'border-(--color-outline)/40'
            "
          >
            <div class="flex-1 flex flex-col">
              <span class="text-[13px] font-bold text-(--color-on-surface)">
                Estimasi Berat Total <TandaWajib />
              </span>
        ...
        <section class="flex flex-col gap-3.5">
          <h2 class="text-sm font-bold text-(--color-on-surface)">
            Jadwal Pengambilan <TandaWajib />
          </h2>
          <div class="grid grid-cols-2 gap-3">
            <DatePickerField v-model="tanggal" wajib :ditandai="ditandai" />
            <TimePickerField v-model="waktu" wajib :ditandai="ditandai" />
          </div>
        </section>
      ...
      <AlertLengkapi :tampil="alertTampil" :kurang="kurang" @tutup="alertTampil = false" />
    </div>
  </template>
</template>
