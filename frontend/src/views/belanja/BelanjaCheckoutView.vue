...
import { BIAYA_LAYANAN, labelJarak, ongkirUntuk } from '@/lib/belanja/ongkir'
...
import { LANGGANAN, PROMO, hitungPromo, promoBerikutnya, type KonteksPromo } from '@/lib/promo/promoBelanja'
import { KATALOG_BY_KATEGORI, adaPromo, hargaFinal, hargaNormal } from '@/lib/belanja/belanjaKatalog'
...
/*
 * Daftar metode, aturan saldo, dan lembarnya hidup di SheetMetodeBayar —
 * dipakai bersama layar konfirmasi BisaKirim. Yang tinggal di sini hanya
 * pilihannya sendiri.
 */
...
<template>
  ...
  <template v-else>
    <div class="min-h-dvh w-full bg-(--color-surface-container) text-(--color-on-surface) pb-40">
      ...
      <!--
        Lembar metode pembayaran. Komponen bersama dengan BisaKirim: daftar ini
        menyimpan aturan saldo dan aktivasi, dan dua salinan berarti aturannya
        diperbaiki di satu layar lalu tetap salah di layar lain.
      -->
      <SheetMetodeBayar v-model="metodeDipilih" v-model:buka="sheetOpen" :total="totalTagihan" />
    </div>
  </template>
</template>
